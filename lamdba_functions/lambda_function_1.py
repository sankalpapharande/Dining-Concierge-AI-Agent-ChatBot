# for handling json like objects
import json
# for communication with the sqs
import boto3
import botocore
# for handing date and time slots
...
NUMS = {'one':'1', 'two':'2','three':'3','four':'4','five':'5','six':'6', 'seven':'7', 'eight':'8', 'nine':'9','ten':'10', 'eleven':'11', 'twelve':'12', 'thirteen':'13', 'fourteen':'14', 'fifteen':'15', 'sixteen':'16','seventeen':'17','eighteen':'18', 'nineteen':'19', 'twenty':'20'}
...
def DiningSuggestionsIntent(event):
    ...
    if event["sessionState"]["intent"]["slots"]["Location"]:
        ...
        if (event["sessionState"]["intent"]["slots"]["Location"]["value"]["originalValue"]).lower() not in places:
            ...
            restaurant_info["Location"] = event["sessionState"]["intent"]["slots"]["Location"]["value"]["interpretedValue"]
    else:
        intent_to_return["sessionState"]["dialogAction"]["slotToElicit"] = "Location"
    ...
    if event["sessionState"]["intent"]["slots"]["DiningDate"]:
        ...
        restaurant_info["DiningDate"] = event["sessionState"]["intent"]["slots"]["DiningDate"]["value"]["interpretedValue"]
    else:
        intent_to_return["sessionState"]["dialogAction"]["slotToElicit"] = "DiningDate"
    ...
    if event["sessionState"]["intent"]["slots"]["DiningTime"]:
        ...
        restaurant_info["DiningTime"] = event["sessionState"]["intent"]["slots"]["DiningTime"]["value"]["interpretedValue"]
    else:
        intent_to_return["sessionState"]["dialogAction"]["slotToElicit"] = "DiningTime"
    ...
    if event["sessionState"]["intent"]["slots"]["NumPeople"]:
        ...
        if (int(x)<0):
            ...
            restaurant_info["NumPeople"] = event["sessionState"]["intent"]["slots"]["NumPeople"]["value"]["interpretedValue"]
    else:
        intent_to_return["sessionState"]["dialogAction"]["slotToElicit"] = "NumPeople"
    ...
    if event["sessionState"]["intent"]["slots"]["PhoneNo"]:
        ...
        if (len(phonenum) != 10) or (not phonenum.isnumeric()):
            ...
            intent_to_return["messages"][0]["content"] = "Oops! That seems like a wrong phone number. Can you pls enter again?"
            return intent_to_return
        else:
            restaurant_info["PhoneNo"] = event["sessionState"]["intent"]["slots"]["PhoneNo"]["value"]["interpretedValue"]
    ...
    if 'Item' in user_table_response.keys():
        ...
        previous_message = "These are few suggestions based on your previous search for cuisine: {}  and location : {}: ".format(previous_cuisine, previous_location) + "\n"+ previous_message
        return {
         "sessionState": {
             "dialogAction": {
                 "type": "ElicitIntent"
