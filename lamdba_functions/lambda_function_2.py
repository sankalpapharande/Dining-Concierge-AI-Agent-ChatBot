...
def lambda_handler(event, context):
    ...
    aws_auth = ('open-search-user', 'Sankalp1!3#')
    ...
    #restaurant_information_df = pd.DataFrame(columns=['id', 'name', 'rating', 'review_count', 'address', 'zip_code'])
    ...
    for i in range(0, no_of_suggestions):
        ...
        if 'Item' in response.keys():
            ...
            'name':restaurant_info['name'], 'rating':float(restaurant_info['rating']), 'address': restaurant_info['address'],
            'review_count':float(restaurant_info['review_count']), 'zip_code':restaurant_info['zip_code']}
            counter = counter + 1
        else:
    ...
    #Message formation:
    opening_message = " Hello! Here are my {} restaurant suggestions for {} people, for the date {} at time {}: ".format(cuisine, no_of_people, dinint_date, dining_time)
    restaurant_message = ""
    for i in range(0, counter):
        restaurant_message = restaurant_message + "{}. {}, located at {}, ".format(i+1, each_restaurant_info[i]['name'], each_restaurant_info[i]['address'])
    ...
    # Stroing Previous Recommendations:
    table_name = "user-information"
    db_client =  boto3.resource('dynamodb')
    user_table = db_client.Table(table_name)
    
    # push restaurant search info to dynamodb
    row_to_be_added = {'phone_no': user_phone_number, 'previous_message': restaurant_message, 'location': location, 'cuisine': cuisine}
    ...
    # total_message = "Hello! Here are my mexican restaurant suggestions for 13 people, for today at 20:00: 1. The Maze, located at 32 W 32nd StFl 3. Enjoy your meal!"
    sns_client = boto3.client('sns', region_name="us-west-2")
    sns_response = sns_client.publish(TargetArn="arn:aws:sns:us-west-2:201565561775:restaurant", Message=json.dumps({'default': json.dumps(total_message)}), MessageStructure='json')
